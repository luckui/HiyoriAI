...
const minecraftGoalTool: ToolDefinition<MinecraftGoalParams> = {
  schema: {
    ...
    function: {
      name: 'minecraft_goal',
      description:
        'Manage Hiyori\'s one current Minecraft gameplay goal. Use set for a goal that needs multiple verified game actions; set also replaces the current goal atomically. Use pause/resume/cancel/status for that goal. This tool does not need an ID.',
      parameters: {
        type: 'object',
        properties: {
          action: {
