...
import { listCodexProjects, listCodexProjectTasks, resolveCodexProject } from '../codexProjectDiscovery';

async function writeSession(root: string, id: string, cwd: string, updatedAt: Date, firstPrompt: string): Promise<void> {
  ...
  const lines = [
    JSON.stringify({
      type: 'session_meta',
      payload: { id, cwd, timestamp: updatedAt.toISOString() },
    }),
    JSON.stringify({
      type: 'response_item',
      payload: {
